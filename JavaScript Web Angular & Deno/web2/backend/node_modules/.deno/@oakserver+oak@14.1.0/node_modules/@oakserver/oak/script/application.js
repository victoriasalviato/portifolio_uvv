...
class Application extends EventTarget {
    ...
    set keys(keys) {
        if (!keys) {
            ...
            this.#keys = new deps_js_1.KeyStack(keys);
        }
        else {
            this.#keys = keys;
        }
    ...
    #getComposed() {
        if (!this.#composedMiddleware) {
            this.#composedMiddleware = (0, middleware_js_1.compose)(this.#middleware);
        }
    ...
    async #handleRequest(request, secure, state) {
        let context;
        try {
            context = new context_js_1.Context(this, request, this.#getContextState(), { secure, ...this.#contextOptions });
        }
        catch (e) {
        ...
        (0, deps_js_1.assert)(context, "Context was not created.");
        const { promise, resolve } = (0, util_js_1.createPromiseWithResolvers)();
    ...
    fetch = async (request, _env, _ctx) => {
        ...
        if (!NativeRequestCtor) {
            const { NativeRequest } = await Promise.resolve().then(() => tslib_1.__importStar(require("./http_server_native_request.js")));
        ...
        const context = new context_js_1.Context(this, contextRequest, this.#getContextState(), this.#contextOptions);
        try {
            await this.#getComposed()(context);
    ...
    handle = (async (request, secureOrAddr, secure = false) => {
        ...
        (0, deps_js_1.assert)((0, util_js_1.isNetAddr)(secureOrAddr) || typeof secureOrAddr === "undefined");
        if (!NativeRequestCtor) {
            const { NativeRequest } = await Promise.resolve().then(() => tslib_1.__importStar(require("./http_server_native_request.js")));
            NativeRequestCtor = NativeRequest;
        }
        ...
        const context = new context_js_1.Context(this, contextRequest, this.#getContextState(), { secure, ...this.#contextOptions });
        try {
            await this.#getComposed()(context);
        }
        catch (err) {
    ...
    async listen(options = { port: 0 }) {
        ...
        if (!this.#serverConstructor) {
            if (!DefaultServerCtor) {
                const { Server } = await ((0, util_js_1.isBun)()
                    ? Promise.resolve().then(() => tslib_1.__importStar(require("./http_server_bun.js"))) : (0, util_js_1.isNode)()
                    ? Promise.resolve().then(() => tslib_1.__importStar(require("./http_server_node.js"))) : Promise.resolve().then(() => tslib_1.__importStar(require("./http_server_native.js"))));

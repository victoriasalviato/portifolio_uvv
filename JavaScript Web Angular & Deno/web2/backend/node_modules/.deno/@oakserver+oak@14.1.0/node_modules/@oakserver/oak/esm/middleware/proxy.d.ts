...
type Fetch = (input: dntShim.Request, init: {
    context: Context;
}) => Promise<dntShim.Response>;
type ProxyMatchFunction<R extends string, P extends RouteParams<R> = RouteParams<R>, S extends State = Record<string, any>> = (ctx: Context<S> | RouterContext<R, P, S>) => boolean;
type ProxyMapFunction<R extends string, P extends RouteParams<R>> = (path: R, params?: P) => R;
type ProxyHeadersFunction<S extends State> = (ctx: Context<S>) => dntShim.HeadersInit | Promise<dntShim.HeadersInit>;
type ProxyRouterHeadersFunction<R extends string, P extends RouteParams<R>, S extends State> = (ctx: RouterContext<R, P, S>) => dntShim.HeadersInit | Promise<dntShim.HeadersInit>;

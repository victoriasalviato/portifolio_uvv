...
async function readRange(file, range) {
    ...
    while (length) {
        ...
        (0, deps_js_1.assert)(nread !== null, "Unexpected EOF encountered when reading a range.");
        (0, deps_js_1.assert)(nread > 0, "Unexpected read of 0 bytes while reading a range.");
        (0, deps_js_1.copyBytes)(p, result, off);
        off += nread;
        length -= nread;
        (0, deps_js_1.assert)(length >= 0, "Unexpected length remaining.");
    }
    return result;
}
    ...
    constructor(file, type, ranges, size, boundary) {
        super({
            pull: async (controller) => {
                ...
                controller.enqueue((0, deps_js_1.concat)([this.#preamble, rangeHeader, bytes]));
            },
        });
